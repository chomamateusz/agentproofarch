...
:::note[Amended 2026-08-03 — the build seeds too]
Point 3 shipped migrations at build time but left `db:seed` a local-only command, so a deployed database kept whatever fixture it was first seeded with. When the published demo password was rotated, every deployed environment kept the old one — the credentials in the README, on the login page and in `smoke:remote`'s defaults were false, and `post-deploy-smoke` failed with 401 on exactly that.

`vercel-build` is now `db:migrate && db:seed && build`, so the fixture converges on **every** deployment. Unguarded rather than production-only: point 2 gives each deployment class its own Neon branch, so no deployment can write another environment's data, and `post-deploy-smoke` drives the same published credentials against Preview deployments too. The seed inserts and updates fixtures only — it deletes nothing, so visitor-created rows survive.
:::
...
3. **Migrations at build time.** The build runs `db:migrate` against the environment's own database before building the SPA, so previews always test the PR's schema on a disposable branch. Staging and production are **forward-only**: destructive changes ship expand → contract across two deploys. *(Amended 2026-08-03: the build runs `db:seed` too — see the note below.)*
4. **Entry**: `demo/api/index.ts` exports a node-style handler through `getRequestListener` from `@hono/node-server` (with `NODEJS_HELPERS=0`); `vercel.json` routes `/api/*` to the function and everything else to the static SPA build with an SPA fallback. Root directory is `demo`.
...
- **US-020 — the Vercel Domains API `DomainPort` — is built**, and its production add path was confirmed live on 2026-07-29 by one owner-supervised run held as a dated adjudication record; its automated tests stay stubbed-`fetch` and CI holds no provider token. That run also carried one pre-verification `domain check`; live acceptance of `check` after verification, and of `remove`, remains unrecorded; see [US-020 status](../operations/self-host-and-domains.md#us-020-production-add-confirmed-live).
:::
