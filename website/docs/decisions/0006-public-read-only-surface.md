...
### 1. A structurally distinct route group
...
### 2. Before identity, never authorize
...
### 3. Content version — derived, not stored
...
### 4. The version is a cache key, not a content selector
...
### 5. One cache helper, errors always `no-store`
...
### 6. CORS on the public group only
