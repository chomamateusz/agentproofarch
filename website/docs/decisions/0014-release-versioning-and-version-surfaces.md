...
bumped only in the owner-approved promotion pull request. Today's unprefixed
...
2. Promotion is the release event. `pnpm run release -- <bump>` updates the
   manifest, inserts a changelog marker, and freezes docs on a major. It never
   commits, tags, or pushes. `tag-release` tags the merged `production` commit
   and refuses to move an existing tag.
...
5. `website/docs/**` is **Next**. A major promotion freezes
   `website/versioned_docs/version-<major>.x/`, which becomes the released
   documentation.
...
| Bump only at promotion | n/a | n/a | n/a | release PR owns the bump; tags never move |
...
| Major docs snapshot | n/a | n/a | n/a | major release PR contains the snapshot |

Four residuals remain explicit:

- a local gate cannot tell a legitimate release bump from a bump on an ordinary
  `main` pull request;
