...
*Read this when the [Quickstart](./quickstart.md) hit something its own short table does not cover. Every row here is a real failure with a real fix — dates and PRs included where an incident produced one.*
...
longer tail — mostly the archaeology of older checkouts and shared machine-wide
Docker state.
