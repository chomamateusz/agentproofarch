...
*Read this if you want to know which jobs run, which ones block a merge, and which deliberately do not. This page is the single source for the required set, the non-required set, and the composition of `check`.*

Every gate runs from a clean `pnpm install --frozen-lockfile` in CI, on every change — and the enforcers themselves are enforced. That is not caution for its own sake. Five consecutive deploy-config failures (PRs #10–#15) shipped with typecheck, lint and tests all green, and production was broken every time. Three more incidents traced a green *local* run to a stale `node_modules` or database rather than the committed tree ([ADR-0004](../decisions/0004-no-exceptions-enforcement.md)). Those eight failures are why the foundation's central claim — *static-green is not done* — is enforced by machinery instead of asserted.
...
This section is the single source for the non-required set — other pages link here instead of counting jobs themselves.

One job used to sit in this list and left it: **`ai-review` graduated to a required `main-gates` check on 2026-07-26**, after accumulating a verdict track record ([the `ai-review` gate](./ai-review-gate.md)).

Today, three jobs run and report without blocking, and one external app reviews without any job at all. Each non-required status is a stated design decision, not an oversight.
...
On failure `visual` uploads `demo/test-results` as a `visual-diff` artifact (7-day retention), because a developer on macOS gets no local comparison at all: baselines are platform-scoped and `ignoreSnapshots` is on for every non-linux platform.

Generate baselines only through the separate `visual-baselines` workflow (`workflow_dispatch`, `update: true`). It re-renders in linux CI, then **re-runs the suite as a comparison against what it just wrote** before uploading the PNGs. An authoring run that died before the harness booted therefore cannot ship an empty or partial baseline set.
...
`ai-review.yml` is a fail-closed AI diff review, and a **required `main-gates` check since 2026-07-26**. The model reviews only the PR diff against this repo's doctrine, with read-only tools, and returns a `PASS`/`FAIL` verdict; the workflow — never the model — sets the exit code. The design goal is one sentence: **"could not verify" and "verified safe" must never collapse to the same colour.** A review that cannot run — limits hit, tool unavailable, timeout, missing token — is red, exactly like a found defect. Only a positive `PASS` is green.
