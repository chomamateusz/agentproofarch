...
This drives live production, so it runs under the production smoke-account doctrine: a dedicated canary tenant, never `db:seed` against a real database, and a drive that does not poison itself. The workflow reads `SMOKE_EMAIL` and `SMOKE_PASSWORD` from CI secrets; each missing secret falls back to its public demo value, so with neither configured the smoke uses `demo@agentproofarch.dev` / `demo1234`.

The workflow enforces the concurrency half of that doctrine with one per-environment group set to **`cancel-in-progress: false`**. A running smoke is a live production verification and must finish rather than be pre-empted, while serialization prevents two runs racing on the shared canary tenant. GitHub keeps only the newest pending run in a concurrency group, so an intermediate smoke can be superseded while another is running; that is acceptable because the newer deployment replaces the older one.
