...
- **Migrations run at build time** against that environment's own database (`vercel-build` = `pnpm run db:migrate && pnpm run db:seed && pnpm run build`). Previews migrate their ephemeral branch, which is always safe. Staging and production are **forward-only**: destructive changes ship as two deploys, expand → contract. The migration *sequence* is mechanically gated — `pnpm run doc-lint` runs `lintMigrations`, which fails the build on a duplicate, gapped or reordered migration.
- **The demo fixture self-heals on every deployment.** The same build step then runs `db:seed`, so the deployed database converges on the credentials this project publishes (`demo@agentproofarch.dev` / `demo-agentproof-1234`) instead of keeping whatever it was first seeded with. Before this, rotating the published password left every deployed environment contradicting its own README and `post-deploy-smoke` failing 401. The seed is convergent — `onConflictDoNothing` inserts plus a password update on the demo account, no deletes — so **visitor-created rows survive**; anyone can add a todo on the live demo and the next deploy will not remove it. It runs on every deployment class rather than production only, because each one owns its Neon branch (nothing can write another environment's data) and `post-deploy-smoke` drives the same published credentials against previews too.
