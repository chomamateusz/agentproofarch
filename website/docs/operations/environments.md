...
*Read this if you are promoting a release, or want to know why an agent cannot.*

Only the owner can put code into production, and that is a property of the environment — nothing an agent is asked to remember. Once agents write most of the code, *who can put code into production* stops being a policy question and becomes an architectural one. The same commit flows feature branch → preview → `main` (staging) → `production`; only environment variables differ. The single edge that reaches production is a pull request the **owner** alone can approve, from a device the agent does not control. Two GitHub rulesets with empty bypass lists are what make that a wall rather than a wish.
...
- **Not every CI job blocks a merge.** On `main` the required set is the four platform checks plus `ai-review` (armed 2026-07-26 after a verdict track record); the deliberately non-required jobs are listed in [CI gates](./ci-gates.md#deliberately-non-required).
...
- **Migrations run at build time** against that environment's own database (`vercel-build` = `pnpm run db:migrate && pnpm run build`). Previews migrate their ephemeral branch, which is always safe. Staging and production are **forward-only**: destructive changes ship as two deploys, expand → contract. The migration *sequence* is mechanically gated — `pnpm run doc-lint` runs `lintMigrations`, which fails the build on a duplicate, gapped or reordered migration.
