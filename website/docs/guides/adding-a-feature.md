...
*Read this if you are wiring your first feature into the stack.*
...
literally the next step. You are not following a tutorial; you are following the
compiler.
...
**Authorization — the second half of step 4.** The generated use-cases call
...
:::note[The generated page sits below rung 1, not outside the doctrine]
The generated page has no island core at all — it reads server state directly
through `actions`, exactly like the
pre-existing todos page, which puts it one step below rung 1 of the
[state-management ladder](../architecture/client-state.md). That is a deliberate
starting point, not a carve-out from
