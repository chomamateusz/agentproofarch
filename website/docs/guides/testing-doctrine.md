...
|---|---|---|---|---|
| **unit** | `pnpm run test` (inside `check`) | node + jsdom, **no database** | {/*count:test-files*/}105{/*/count*/} test files | every `check`, locally and in CI |
| **integration** | `pnpm run test:integration` | real Postgres, opt-in via `VITEST_INTEGRATION=1` | {/*count:integration-tests*/}51{/*/count*/} tests | the CI `smoke` job — the only one with a database |
...
{/*count:config-regression*/}61{/*/count*/} tests whose subject is the **enforcers**. Two shapes, and the difference is
