...
|---|---|---|---|---|
| **unit** | `pnpm run test` (inside `check`) | node + jsdom, **no database** | 89 test files | every `check`, locally and in CI |
...
57 tests whose subject is the **enforcers**. Two shapes, and the difference is
