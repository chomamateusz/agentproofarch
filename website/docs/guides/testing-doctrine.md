...
|---|---|---|---|---|
| **unit** | `pnpm run test` (inside `check`) | node + jsdom, **no database** | {/*count:test-files*/}91{/*/count*/} test files | every `check`, locally and in CI |
...
{/*count:config-regression*/}57{/*/count*/} tests whose subject is the **enforcers**. Two shapes, and the difference is
