...
*Read this if you are wiring your first feature or scripting the agent loop. Per-command transcripts live in the [CLI command reference](./cli-reference.md).*
...
wired. This page covers the invocation model, the stored config, the envelope and
the exit codes; the [CLI command reference](./cli-reference.md) walks every command
group as it exists in `demo/apps/cli/src/main.ts`.
...
- [Testing doctrine](./testing-doctrine.md) and
  [Errors and API versioning](../architecture/errors-and-api-versioning.md) — the
  gates and the taxonomy behind the exit codes.
