...
1. **Both gates are required CI checks on every PR** (`ci` workflow, on `pull_request` and `push` to `main`):
   - **`check`** — `pnpm install --frozen-lockfile && pnpm run check`, the static gate from a clean install: typecheck, island typecheck, ESLint layer boundaries, `lock-lint`, dependency-cruiser, knip, doc-lint and vitest with coverage.
...
4. **Doc-lint** (`pnpm run doc-lint`, wired into `check`) keeps docs and enforcer configuration in sync **both ways**:
   ...
   - **dead links**: every git-tracked `.md` has its link targets resolved on disk — relative ones against the linking file, site-absolute ones (`/img/…`, the only spelling a live page and its versioned snapshot can share) against `website/static/`, mirroring how Docusaurus serves them; a leading `/` outside the site is an error, because there it means the filesystem root. Build-generated docs are a named exception (today exactly one — `website/docs/changelog.md`, written by `prebuild`), because they are legitimately absent from a clean checkout; the exception is a literal path list, so a typo still fails.
...
- **CI runs only on the canonical repo.** Jobs use `if: github.repository == 'chomamateusz/agentproofarch'` directly or inherit it through a guarded dependency; `approve-visuals` is the transitive case. A fork never spends Actions minutes or fails on missing secrets and services.
