...
| 3 | Email change + re-verify | MISSING | Gated behind `user.changeEmail.enabled`, never set. Compounded by #3 below (member-email refresh unbuilt) — shipping this without that fix would silently desync `members.email`. | table-stakes |
