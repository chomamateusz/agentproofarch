...
|---|---|---|
| `Token-Permissions` | Below 10, deliberately | Every workflow's top level is read-only; every write is job-scoped, and each of the six that hold one cannot do its work without it — `tag-release` and `visual-baselines` (`contents`, to create the tag and to push re-rendered baselines), `approve-visuals` (`actions`, to dispatch that run), `ci`'s `visual-report` (`contents` + `pull-requests`, to push the gallery branch and comment it), `ai-review` (`pull-requests`, to post the verdict), `docs-deploy` (`pages` + `id-token`) and `scorecard` itself (`security-events`, to upload SARIF). Scorecard scores a job-level write as a finding regardless; dropping the capability would break the job, so the score stays where it is |
