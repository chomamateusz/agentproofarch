...
| `Code-Review` | Should rise, still only partly read | [`CODEOWNERS`](../../.github/CODEOWNERS) now routes every path to `@chomamateusz`, so `production-protection`'s code-owner review requirement binds on a real owner rather than on nobody; the rulesets' pull-request requirement itself is still only partly detected |
