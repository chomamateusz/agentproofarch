...
/**
 * Builds the input for the advisory "AI read" verdict (ADR-0013 polish): the
 * changed screenshots copied under sanitized names next to a context file
 * holding their pixel counts and the pull request's file diff. It runs in the
 * `visual-report` job before the model step, on the trusted base commit; the
 * artifact paths it reads are the same sanitized set the gallery publishes.
 * The whole chain is fail-open — the caller treats any failure here as
 * "verdict unavailable", never as a red job.
 */
