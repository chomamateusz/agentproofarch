...
const migrateLegacyConfig = (legacy: z.output<typeof legacyConfigSchema>): CliConfig => {
  ...
  console.error(
    `agentproofarch: migrated ~/.config/agentproofarch/config.json to per-origin profiles (${origin})`,
  );
  return migrated;
};
...
export const loadConfig = (): CliConfig => {
  ...
  try {
    ...
    throw new Error(
      `agentproofarch: could not read ~/.config/agentproofarch/config.json: ${String(error)}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `agentproofarch: invalid ~/.config/agentproofarch/config.json: malformed JSON (${String(error)})`,
    );
  }
  ...
  if (record.success && Object.hasOwn(record.data, 'version')) {
    ...
    throw new Error(
      `agentproofarch: invalid ~/.config/agentproofarch/config.json: ${current.error.issues
...
export const isAgentproofarchRepo = (cwd: string): boolean => {
  ...
  while (true) {
    try {
      ...
      if (parsed.success && parsed.data.name === 'agentproofarch') return true;
    } catch {}
  ...
  const apiSelection =
    input.apiUrl !== undefined
      ...
      : input.env.APP_CLI_API_URL !== undefined
        ...
        : isAgentproofarchRepo(input.cwd)
