...
interface ConfigModule {
  ...
  isAgentproofarchRepo: (cwd: string) => boolean;
...
const repo = join(home, 'renamed-checkout', 'demo');
...
beforeEach(() => {
  ...
  writeFileSync(join(repo, 'package.json'), JSON.stringify({ name: 'agentproofarch' }));
});
...
describe('loadConfig', () => {
  ...
  it('migrates a legacy profile without changing token or tenant bytes', () => {
    ...
    expect(error).toHaveBeenCalledExactlyOnceWith(
      'agentproofarch: migrated ~/.config/agentproofarch/config.json to per-origin profiles (https://example.com)',
    );
  });
  ...
  it('fails loudly on a corrupted legacy field and never resets the file', () => {
    ...
    expect(() => config.loadConfig()).toThrow(
      'agentproofarch: invalid ~/.config/agentproofarch/config.json',
    );
    expect(readFileSync(configFile, 'utf8')).toBe(corrupted);
  });
...
describe('repo detection', () => {
  it('detects the package marker from a renamed checkout child', () => {
    expect(config.isAgentproofarchRepo(repoChild)).toBe(true);
  });
  ...
  it('does not misfire outside the checkout or on a different package name', () => {
    ...
    expect(config.isAgentproofarchRepo(outsideRepo)).toBe(false);
  });
