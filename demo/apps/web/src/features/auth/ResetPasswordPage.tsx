 ...
 * `error=INVALID_TOKEN` when the link was already used, expired or tampered with.
 */
...
export const ResetPasswordPage = () => {
  ...
  // The confirmation is worth reading before the login form replaces it, so the
  // redirect is deferred rather than fired inside `onSuccess`.
  useEffect(() => {
  ...
  if (done) {
    return (
      <Card>
        ...
        <Alert severity="success">Password updated. Taking you to the sign-in form…</Alert>
        <Divider sx={{ mt: '1.4rem', mb: '0.9rem' }} />
        <Eyebrow variant="caption" component="p">
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'grid', placeItems: 'center', p: '1.5rem' }}>
      ...
      >
        ...
        <Eyebrow variant="overline" component="p" sx={{ mb: '1.6rem' }}>
          choose a new password · tenant {window.location.hostname}
        </Eyebrow>
        <Stack useFlexGap spacing="1rem">
          <FormControl fullWidth error={Boolean(clientErrors.newPassword)}>
            ...
            {clientErrors.newPassword ? (
              <FormHelperText>{clientErrors.newPassword}</FormHelperText>
            ) : null}
          </FormControl>
          <FormControl fullWidth error={Boolean(clientErrors.confirmPassword)}>
            <FormLabel htmlFor="reset-password-confirm">repeat password</FormLabel>
            <OutlinedInput
            ...
            {clientErrors.confirmPassword ? (
              <FormHelperText>{clientErrors.confirmPassword}</FormHelperText>
            ) : null}
          </FormControl>
          <Button
            type="submit"
