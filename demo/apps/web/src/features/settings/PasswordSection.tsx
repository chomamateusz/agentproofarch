...
export const PasswordSection = () => {
  ...
  const submit = () => {
    ...
    if (!parsed.success) {
      setNewPasswordError(parsed.error.issues[0]?.message ?? 'Enter a valid password');
      return;
    }
    setNewPasswordError(null);
  ...
  return (
    <Paper variant="outlined" sx={{ p: '1.25rem', mt: '1.5rem' }}>
      ...
      <Stack useFlexGap spacing="0.8rem">
        ...
        {changePassword.isSuccess ? <Alert severity="success">Password changed.</Alert> : null}
        {changePassword.isError ? <Alert>{errorText(changePassword.error)}</Alert> : null}
      </Stack>
    </Paper>
  );
};
